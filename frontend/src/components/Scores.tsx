  ...
  return (
    <table className="border-separate border-spacing-0 w-full">
        <thead className='[&>*]:divide-x [&>*]:divide-solid [&>*]:ring sticky top-0 [&>*]:bg-white z-10 w-full'>
          {header?.map((row, i) => (
            <tr className='[&>td]:font-bold [&>td]:text-md [&>td]:text-dark-blue [&>td]:px-2 [&>td]:whitespace-nowrap' key={i}>
        ...
        <tbody className='[&>*]:divide-x [&>*]:divide-solid [&>*]:ring'>
          {values?.map((row, i) => (
            <tr className={`[&>*]:text-sm [&>*]:p-2 ${i % 3 === 0 ? '[&>*]:bg-light-green ': '[&>*]:bg-white'} ${i%3 === 2 ? '[&>*]:border-b-2 [&>*]:border-b-solid [&>*]:border-black' : ''}`} key={i}>
